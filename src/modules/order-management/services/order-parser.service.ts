  ...
  async parseFile(path: string): Promise<any[]> {
    ...
    for (let i = 0; i < Math.min(rows.length, 40); i++) {
      ...
      for (let j = 0; j < row.length; j++) {
        ...
        if (
          (cellValue === 'наименование' ||
            cellValue === 'наименование номенклатуры' ||
            (cellValue.includes('наименование') && cellValue.length < 30)) &&
          foundIndices.name === -1
        ) {
          foundIndices.name = j;
        }
        ...
        if (
          (cellValue === 'кол-во' ||
            ...
            (cellValue.includes('кол') && cellValue.length < 10)) &&
          foundIndices.quantity === -1
        ) {
    ...
    if (
      ...
      throw new BadRequestException(
        'Не найдена строка с заголовками (Код/Артикул, Наименование, Кол-во/Количество)',
      );
    }
