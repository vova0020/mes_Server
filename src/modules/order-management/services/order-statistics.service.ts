...
export class OrderStatisticsService {
            ...
            partsMap.set(part.partId, {
              ...
              stages: part.route.routeStages.map((rs) => {
                ...
                if (rs.stage.finalStage) {
                  ...
                  // Для обычных этапов считаем по общему количеству деталей
                  const totalQuantity = Number(part.totalQuantity);
    ...
    for (const pkg of order.packages) {
      for (const ppp of pkg.productionPackageParts) {
        ...
        if (partsMap.has(part.partId)) {
          ...
          partData.pallets = part.pallets.map((pallet) => ({
            ...
            stages: part.route.routeStages.map((rs) => {
              ...
              if (rs.stage.finalStage) {
                ...
                const assignedPackage = pallet.packageAssignments?.find(
                  (assignment) =>
                    order.packages.some(
                      (pkg) => pkg.packageId === assignment.packageId,
                    ),
                );
                if (assignedPackage) {
                  const packageStatus = order.packages.find(
          ...
          partData.totalDefected =
            defectReclamations._sum.quantity?.toNumber() || 0;
          partData.totalReturned =
            returnMovements._sum.deltaQuantity?.toNumber() || 0;
        }
      }
    }
  ...
  private calculateOrderProgress(order: any, parts: any[]) {
    ...
    parts.forEach((part) => {
      ...
      const productionStages = part.stages.filter(
        (stage) => stage.finalStage === false,
      );

      productionStages.forEach((stage) => {
        totalStageWork += 1;
        completedStageWork += stage.completionPercentage / 100;
      });
    });
