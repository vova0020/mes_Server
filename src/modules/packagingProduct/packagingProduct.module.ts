// src/your/your.module.ts

import { Module } from '@nestjs/common';
import { PackagingController } from './controllers/packagingProduct.controller';
import { PackagingProductService } from './services/packagingProduct.service';
import { SharedModule } from '../../shared/shared.module';
import { WebsocketModule } from '../websocket/websocket.module';
/**
 * Модуль объединяет контроллер и сервис в единый «пакет».
 * Чтобы Nest знал о них, нужно зарегистрировать в imports/app.module.ts.
 */
@Module({
  imports: [SharedModule, WebsocketModule],
  controllers: [PackagingController], // какие контроллеры «живут» в этом модуле
  providers: [PackagingProductService], // какие сервисы инжектятся внутри
})
export class PackagingProductModule {}
