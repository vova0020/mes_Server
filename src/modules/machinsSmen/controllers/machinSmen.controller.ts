...
@ApiTags('Станки со сменами')
@Controller('machinesSmen')
export class MachinSmenController {
  constructor(private readonly MachinSmenService: MachinSmenService) { }
  ...
  async getMachineById(
    ...
    const result = await this.MachinSmenService.getMachineById(id);
    console.log(
  ...
  async updateMachineStatus(
    ...
    const result = await this.MachinSmenService.updateMachineStatus(
      updateDto.machineId,
      updateDto.status,
    );
  ...
  async getSegmentOrders(
    ...
    const result = await this.MachinSmenService.getSegmentOrders(segmentId);
    console.log(`Найдено заказов: ${result.orders.length}`);
    return result;
  }
// !!!!УСтаревший запрос убрать потом
  @Get('order/details')
  @ApiOperation({
  ...
  async getOrderDetails(
    ...
    const result = await this.MachinSmenService.getOrderDetails(
      orderId,
      segmentId,
    );
  ...
  async getPalletsByDetailId(
    ...
    const result = await this.MachinSmenService.getPalletsByDetailId(detailId, segmentId);
